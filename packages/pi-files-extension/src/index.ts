 ...
 * Files Extension
 *
 * Based on https://github.com/mitsuhiko/agent-stuff/blob/main/pi-extensions/files.ts
 *
...
const extractFileReferencesFromEntry = (entry: SessionEntry): string[] => {
  if (entry.type === "message") {
    ...
    if ("content" in msg) {
      return extractFileReferencesFromContent(msg.content);
    }
    return [];
  }

  // Handle custom_message entries that may exist in some pi versions
  const anyEntry = entry as unknown as Record<string, unknown>;
  if (anyEntry.type === "custom_message" && "content" in anyEntry) {
    return extractFileReferencesFromContent(anyEntry.content);
  }
...
// ---------------------------------------------------------------------------
// Session file reference collection
// ---------------------------------------------------------------------------
...
const buildFileEntries = async (
  ...
  // Add git-tracked and untracked files
  ...
  // Add files from git status that weren't in ls-files
  ...
  // Sort: dirty first, then in-repo, then session-changed, then by recency, then referenced, then alpha
