...
`,
  ...
  it.each([
    ...
    "routes create-fix-forward with $name to attention without creating child tasks",
    async ({ decision, expectedReason }) => {
      ...
      const artifactDir = path.join(
        repoRoot,
        DEFAULT_CONFIG.storage.runtimeDir,
        "scope-validation",
      );
      const artifacts = await readdir(artifactDir);
      const attentionLog = await readFile(
        path.join(
          artifactDir,
          artifacts.find((entry) => entry.includes("-attention-log")) as string,
        ),
        "utf8",
      );
