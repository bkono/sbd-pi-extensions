...
describe("GDScript read workflows", () => {
  ...
  it("renders maps and reads GDScript symbols through the registered mapper", async () => {
    ...
    const { __setHashlineSettingsPathsForTest: setFallbackSettingsPathsForTest } = await import(
      "../src/hashline-settings.js"
    );
    setFallbackSettingsPathsForTest({
      globalSettingsPath: join(fallbackRoot, "missing.json"),
      projectSettingsPath: fallbackSettingsPath,
    });
    const nullMapper = vi.fn(async () => null);
    vi.doMock("../src/readmap/mappers/gdscript.js", () => ({
      MAPPER_VERSION: 1,
      gdscriptMapper: nullMapper,
    }));
