...
describe("WASM parser loader", () => {
  ...
  it("returns null under Bun without initializing", async () => {
    ...
    const { getWasmParser, __resetWasmParserLoaderForTests } = await import(
      "../src/readmap/parser-loader.js"
    );
  ...
  it("memoizes init and language loads", async () => {
    ...
    expect(mocks.MockLanguage.load).toHaveBeenCalledWith(
      expect.stringMatching(/tree-sitter-wasms[/\\]out[/\\]tree-sitter-rust\.wasm$/),
    );
    a?.delete();
    b?.delete();
  });
  ...
  it("maps c-header to cpp grammar", async () => {
    ...
    expect(mocks.MockLanguage.load).toHaveBeenCalledWith(
      expect.stringMatching(/tree-sitter-wasms[/\\]out[/\\]tree-sitter-cpp\.wasm$/),
    );
  });
});
