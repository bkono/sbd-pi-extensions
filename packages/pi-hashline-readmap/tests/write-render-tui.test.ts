...
describe("write TUI renderer", () => {
  it("shows created and overwritten state before final diff without mutating details", () => {
    ...
    const t = tool();
    expect(
      textOf(t.renderCall({ path: "tmp/file.txt", content: "one\ntwo\nthree" }, theme, {})),
    ).toBe("write tmp/file.txt (3 lines • 13 B)");
    const baseDiffData = {
      version: 1,
    ...
    const created: any = {
      ...
      details: {
        ...
        ptcValue: { tool: "write", diffData: { sentinel: true }, lines: createdPtcLines },
      },
    };
    ...
    const createdExpanded = textOf(
      t.renderResult(created, { expanded: true, width: 80 }, theme, { expanded: true, width: 80 }),
      80,
    );
    ...
    const overwritten: any = {
      ...
      details: {
        ...
        diffData: overwrittenDiffData,
        ptcValue: { tool: "write" },
      },
    };
    const overwrittenRendered = textOf(
      t.renderResult(overwritten, { expanded: true, width: 80 }, theme, {
        expanded: true,
        width: 80,
      }),
      80,
    ...
    const collapsed: any = {
      content: created.content,
      details: { ...created.details, writeState: "overwritten", diffData: overwrittenDiffData },
    };
    expect(textOf(t.renderResult(collapsed, {}, theme, {}))).toBe(
      "↳ overwritten • Ctrl+O to expand",
    );
  });
  ...
  it("wraps the write path title in an OSC 8 hyperlink when supported", () => {
    ...
    setCapabilities({ images: null, trueColor: true, hyperlinks: false });
    expect(textOf(tool().renderCall(args, theme, { cwd }))).toBe(
      "write tmp/file.txt (2 lines • 7 B)",
    );
  });
  ...
  it("keeps write error summaries visible and expands details", () => {
    ...
    const failed: any = {
      ...
      details: { ptcValue: { ok: false } },
    };
    expect(textOf(t.renderResult(failed, {}, theme, {}))).toBe("↳ Permission denied");
    expect(textOf(t.renderResult(failed, { expanded: true }, theme, { expanded: true }))).toContain(
      "full detail",
    );
  });
  ...
  it("collapses the pending preview to just the call line once execution has started", () => {
    ...
    const beforeCreateText = textOf(
      t.renderCall({ path: "new.txt", content: "one\ntwo" }, theme, beforeCreate),
    );
    expect(beforeCreateText).toContain("↳ pending create");

    const beforeOverwrite: any = {
      cwd,
      state: {},
      invalidate: vi.fn(),
      expanded: true,
    ...
    const beforeOverwriteText = textOf(
      t.renderCall({ path: "old.txt", content: "old\nnew" }, theme, beforeOverwrite),
    );
    ...
    const afterOverwriteText = textOf(
  ...
  it("renders the create content preview when theme.fg uses `this` (regression: bind theme.fg)", () => {
    ...
    class MethodTheme {
      ...
      fg(style: string, text: string) {
        if (!this.tag) throw new TypeError("this is undefined");
    ...
    const created: any = {
      content: [{ type: "text", text: "1:abc|one\n2:def|two" }],
      details: { writeState: "created", ptcValue: { tool: "write", lines: ptcLines } },
    };
    let rendered: any;
    expect(() => {
      rendered = t.renderResult(created, { expanded: true, width: 80 }, methodTheme, {
        expanded: true,
        width: 80,
      });
    }).not.toThrow();
