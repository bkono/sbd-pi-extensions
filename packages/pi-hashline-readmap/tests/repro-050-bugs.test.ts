...
async function callReadTool(params: { path: string; offset?: number; limit?: number }) {
  ...
  return capturedTool.execute("tc", params, new AbortController().signal, () => {}, {
    cwd: packageRoot,
  });
}
...
beforeAll(() => {
  ...
  writeFileSync(
    join(tmpDir, "no-nul.bin"),
    Buffer.from([
      0x83, 0xc8, 0x1d, 0x9f, 0x5f, 0x12, 0x1c, 0x26, 0x38, 0xe8, 0x95, 0x05, 0x63, 0xdc, 0x68,
      0xb1,
    ]),
  );
  ...
  writeFileSync(
    join(tmpDir, "high-bit.bin"),
    Buffer.from(Array.from({ length: 10 }, (_, i) => 246 + i)),
  );
...
describe("Issue #047: grep silently returns 0 matches on non-NUL binary with ASCII", () => {
  it("grep on binary-needle.bin returns 0 matches with no warning", async () => {
    ...
    const result = await callGrepTool({
      pattern: "NEEDLE",
      path: join(tmpDir, "binary-needle.bin"),
      literal: true,
    });
    const output = getText(result);
  ...
  it("ripgrep raw behavior: non-NUL binary silently exits 1 with no output", () => {
    ...
    try {
      rawStdout = execFileSync(
        "rg",
        ["--line-number", "NEEDLE", join(tmpDir, "binary-needle.bin")],
        { encoding: "utf8" },
      );
    } catch (e: any) {
      rawStdout = e.stdout ?? "";
