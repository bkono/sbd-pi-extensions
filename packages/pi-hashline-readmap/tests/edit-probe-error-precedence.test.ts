...
describe("F2 — replace_symbol probe-pass error precedence", () => {
      ...
      "c",
      {
        ...
        edits: [
          ...
          },
          {
            replace_symbol: {
              symbol: "add",
              new_body: "export function add(a: number, b: number) { return a + b + 1; }",
            },
          },
  ...
  it("AC 4: generateMapFromContent called once per replace_symbol edit in a multi-symbol batch", async () => {
    ...
    writeFileSync(
      fp,
      ["export function add() { return 1; }", "export function sub() { return 3; }", ""].join("\n"),
    );
    ...
    expect(readFileSync(fp, "utf-8")).toBe(
      ["export function add() { return 2; }", "export function sub() { return 4; }", ""].join("\n"),
    );
  });
  ...
  it("rejects duplicate replace_symbol ranges before applying any replacement", async () => {
    ...
    const fp = join(dir, "x.ts");
    writeFileSync(fp, ["export function add() {", "  return 1;", "}", ""].join("\n"));
    ...
    expect(readFileSync(fp, "utf-8")).toBe(
      ["export function add() {", "  return 1;", "}", ""].join("\n"),
    );
  });
