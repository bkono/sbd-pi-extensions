...
import { compressBuildToolsOutput, isBuildToolsCommand } from "../src/rtk/build-tools.js";
...
describe("isBuildToolsCommand", () => {
  ...
  it("returns false for npm run build", () =>
    expect(isBuildToolsCommand("npm run build")).toBe(false));
  ...
  it("returns false for rsync", () =>
    expect(isBuildToolsCommand("rsync -av src/ dst/")).toBe(false));
});
...
describe("compressBuildToolsOutput", () => {
  ...
  it("strips CMake progress percentage lines", () => {
    const lines =
      Array(15).fill("[ 10%] Building CXX object CMakeFiles/foo.dir/main.cpp.o\n").join("") +
      "done\n";
    const result = compressBuildToolsOutput(lines)!;
  ...
  it("strips Maven [INFO] Downloading lines", () => {
    const lines =
      Array(15).fill("[INFO] Downloading: https://repo.maven.apache.org/foo\n").join("") +
      "[INFO] BUILD SUCCESS\n";
    const result = compressBuildToolsOutput(lines)!;
  ...
  it("preserves error: lines", () => {
    const lines =
      Array(10).fill("noise\n").join("") +
      "src/main.cpp:10:5: error: use of undeclared identifier\n";
    const result = compressBuildToolsOutput(lines)!;
