...
`,
    ...
    const readRes = await readTool.execute("c", { path: fp, symbol: "bar" }, undefined, undefined, {
      cwd: dir,
    });
    ...
    const editMessage =
      editRes.details?.ptcValue?.error?.message ?? editRes.content?.[0]?.text ?? "";
  ...
  it("replace_symbol not-found error matches the read tool's not-found warning verbatim", async () => {
    ...
    const editRes = await editTool.execute(
      "c",
      {
        path: fp,
        edits: [
          { replace_symbol: { symbol: "missing", new_body: "export function missing() {}" } },
        ],
      },
      undefined,
      undefined,
  ...
  it("emits name-mismatch warning when new_body declares a different name", async () => {
    ...
    const res = await tool.execute(
      "c",
      {
        path: fp,
        edits: [
          { replace_symbol: { symbol: "add", new_body: "export function plus() { return 2; }" } },
        ],
      },
      undefined,
      undefined,
    ...
    const res = await tool.execute(
      "c",
      {
        ...
        edits: [
          ...
          {
            set_line: {
              anchor: `2:${computeLineHash(2, "  return a + b;")}`,
              new_text: "  return 0;",
            },
          },
        ],
    ...
    try {
      ...
      const res = await tool.execute(
        "c",
        {
          path: fp,
          edits: [
            { replace_symbol: { symbol: "sub", new_body: "export function sub() { return -1; }" } },
          ],
        },
        undefined,
        undefined,
