...
function capture(register: (pi: any, options?: any) => any): any {
  let registered: any;
  register(
    {
...
describe("search TUI renderers", () => {
  it("renders compact grep summaries and expanded details", () => {
    ...
    expect(textOf(grep.renderCall({ pattern: "diffData", path: "src" }, theme))).toBe(
      "grep /diffData/ in src",
    );
    const result = {
      content: [{ type: "text", text: "src/a.ts:1:abc|diffData" }],
      details: {
        ptcValue: {
          tool: "grep",
          summary: false,
          totalMatches: 1,
          records: [{ path: `${process.cwd()}/src/a.ts`, kind: "match" }],
        },
      },
    };
    expect(textOf(grep.renderResult(result, {}, theme, { cwd: process.cwd() }))).toBe(
      "↳ 1 match returned • Ctrl+O to expand",
    );
    expect(
      textOf(
        grep.renderResult(result, { expanded: true }, theme, {
          expanded: true,
          cwd: process.cwd(),
        }),
      ),
  ...
  it("renders compact ast_search summaries", () => {
    ...
    expect(
      textOf(sg.renderCall({ pattern: "console.log($A)", path: "src", lang: "typescript" }, theme)),
    ).toBe("ast_search /console.log($A)/ in src (typescript)");
    const result = {
      content: [{ type: "text", text: "src/a.ts\n1:abc|console.log(a)" }],
      details: {
        ptcValue: {
          tool: "ast_search",
          files: [{ path: `${process.cwd()}/src/a.ts`, lines: [{ line: 1 }] }],
        },
      },
    };
    expect(textOf(sg.renderResult(result, {}, theme, { cwd: process.cwd() }))).toBe(
      "↳ 1 match in 1 file • Ctrl+O to expand",
    );
  });
  ...
  it("uses summary grammar for pending and error states", () => {
    ...
    expect(textOf(grep.renderResult({ content: [] }, {}, theme, { isPartial: true }))).toBe(
      "↳ pending search",
    );
    expect(textOf(grep.renderResult(failed, {}, theme, {}))).toBe("↳ first line");
    expect(
      textOf(grep.renderResult(failed, { expanded: true }, theme, { expanded: true })),
    ).toContain("full detail");

    expect(textOf(sg.renderResult({ content: [] }, {}, theme, { isPartial: true }))).toBe(
      "↳ pending search",
    );
    expect(textOf(sg.renderResult(failed, {}, theme, {}))).toBe("↳ first line");
    expect(
      textOf(sg.renderResult(failed, { expanded: true }, theme, { expanded: true })),
    ).toContain("full detail");
  });
});
