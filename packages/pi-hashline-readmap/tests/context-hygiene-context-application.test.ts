...
describe("context hygiene context application", () => {
  it("masks stale read provider context after same-file edit", () => {
    ...
    const liveEdit = toolResult("edit-file", "edit", "edit succeeded", {
      ptcValue: { tool: "edit" },
    });
    ...
    expect(applied[0].content).toEqual([
      {
        type: "text",
        text: "[Stale read result — this earlier read was superseded by a later file change; nothing is wrong with read. Run read again for current content.]",
      },
    ]);
  ...
  it("masks stale grep provider context after same-file edit", () => {
    ...
    expect(applied[0].content).toEqual([
      {
        type: "text",
        text: "[Stale grep result — this earlier grep was superseded by a later file change; nothing is wrong with grep. Run grep again for current matches.]",
      },
    ]);
    expect(applied[0].details).toMatchObject({
      contextHygieneStale: {
  ...
  it("masks stale ast_search provider context after same-file edit", () => {
    ...
    expect(applied[0].content).toEqual([
      {
        type: "text",
        text: "[Stale ast_search result — this earlier ast_search was superseded by a later file change; nothing is wrong with ast_search. Run ast_search again for current matches.]",
      },
    ]);
    expect(applied[0].details).toMatchObject({
      contextHygieneStale: {
