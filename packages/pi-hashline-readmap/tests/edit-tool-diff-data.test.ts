...
describe("edit tool diffData", () => {
  ...
  it("returns diffData in details and ptcValue for compact anchor edits without changing legacy fields", async () => {
    ...
    expect(result.details.diff).toMatch(
      /^1:[0-9a-f]{3}\|const value = 1; → 1:[0-9a-f]{3}\|const value = 11;$/,
    );
  ...
  it("returns 1:1 diffData entries for full multi-line edit diffs", async () => {
    ...
    expect(result.details.diffData.entries.some((entry: any) => entry.kind === "context")).toBe(
      true,
    );
  });
  ...
  it("returns replace_symbol blockRanges from resolved symbol ranges", async () => {
    ...
    writeFileSync(
      filePath,
      ["export function greet(firstName: string) {", "  return firstName;", "}"].join("\n"),
      "utf-8",
    );
      ...
      edits: [
        {
          replace_symbol: {
            symbol: "greet",
            new_body: [
              "export function greet(displayName: string) {",
              "  return displayName;",
              "}",
            ].join("\n"),
          },
    ...
    expect(result.details.diffData.blockRanges).toEqual([
      { kind: "remove", startLine: 1, endLine: 3 },
    ]);
  });
});
