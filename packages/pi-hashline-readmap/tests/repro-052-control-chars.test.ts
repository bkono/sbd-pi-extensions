...
async function callReadTool(params: { path: string; offset?: number; limit?: number }) {
  ...
  return capturedTool.execute("test", params, new AbortController().signal, () => {}, {
    cwd: process.cwd(),
  });
}
...
// biome-ignore lint/suspicious/noControlCharactersInRegex: test asserts control characters are escaped.
...
describe("Bug #052: control characters in read/grep output", () => {
  ...
  it("read and grep anchors from escaped output still validate against the raw file", async () => {
    ...
    const grepAnchor = grepText
      .match(/>>5:([0-9a-f]{3})\|/m)?.[0]
      .replace(/^>>/, "")
      .replace(/\|$/, "");
