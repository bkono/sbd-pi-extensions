...
import { compressTransferOutput, isTransferCommand } from "../src/rtk/transfer.js";
...
describe("isTransferCommand", () => {
  ...
  it("returns false for curl", () =>
    expect(isTransferCommand("curl https://example.com")).toBe(false));
  it("returns false for git", () => expect(isTransferCommand("git push")).toBe(false));
  it("returns false for docker cp", () =>
    expect(isTransferCommand("docker cp file container:/path")).toBe(false));
...
describe("compressTransferOutput", () => {
  ...
  it("preserves final summary line with 'sent ... bytes'", () => {
    const lines =
      Array(14).fill("noise progress\n").join("") +
      "sent 1,234 bytes  received 85 bytes  876.00 bytes/sec\n";
    const result = compressTransferOutput(lines)!;
  ...
  it("preserves Connection refused lines", () => {
    const lines =
      ...
      "ssh: connect to host example.com: Connection refused\n";
    const result = compressTransferOutput(lines)!;
