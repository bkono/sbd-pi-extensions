...
import { GENERIC_SUGGESTION, SUGGESTIONS } from "../src/doom-loop-suggestions.js";
...
describe("SUGGESTIONS table", () => {
  ...
  it("contains the exact strings from the issue doc for each tool", () => {
    ...
    expect(SUGGESTIONS.ls).toEqual(["try a different path", "remove the glob filter"]);
  });
  ...
  it("source file carries an upkeep comment", () => {
    const source = readFileSync(
      new URL("../src/doom-loop-suggestions.ts", import.meta.url),
      "utf8",
    );
