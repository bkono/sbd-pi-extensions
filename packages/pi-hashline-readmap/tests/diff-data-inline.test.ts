...
describe("DiffData inline diffs", () => {
  ...
  it("adds inline spans for similar compact rename rows", () => {
    ...
    expect(joinSpans(inline.removeSpans)).toBe(
      "function greet(firstName: string) { return firstName; }",
    );
    expect(joinSpans(inline.addSpans)).toBe(
      "function greet(displayName: string) { return displayName; }",
    );
    expect(
      inline.removeSpans.some((span) => span.kind === "remove" && span.text.includes("firstName")),
    ).toBe(true);
    expect(
      inline.addSpans.some((span) => span.kind === "add" && span.text.includes("displayName")),
    ).toBe(true);
  });
  ...
  it("adds inline spans for a multi-token single-line edit", () => {
    ...
    expect(
      inline.removeSpans.some((span) => span.kind === "remove" && span.text.includes("total")),
    ).toBe(true);
    expect(
      inline.addSpans.some((span) => span.kind === "add" && span.text.includes("subtotal")),
    ).toBe(true);
    expect(
      inline.addSpans.some((span) => span.kind === "add" && span.text.includes("discount")),
    ).toBe(true);
  });
  ...
  it("pairs multi-line replacement rows by hunk-relative position", () => {
    ...
    expect(joinSpans(diffData.inlineDiffs![0]!.removeSpans)).toBe(
      "function one(firstName: string) { return firstName; }",
    );
    expect(joinSpans(diffData.inlineDiffs![0]!.addSpans)).toBe(
      "function one(displayName: string) { return displayName; }",
    );
    expect(diffData.inlineDiffs![1]!.removeLineIndex).toBe(1);
    expect(diffData.inlineDiffs![1]!.addLineIndex).toBe(3);
    expect(joinSpans(diffData.inlineDiffs![1]!.removeSpans)).toBe(
      "function two(lastName: string) { return lastName; }",
    );
    expect(joinSpans(diffData.inlineDiffs![1]!.addSpans)).toBe(
      "function two(familyName: string) { return familyName; }",
    );
  });
  ...
  it("skips token-heavy lines before allocating inline diff tables", () => {
    const oldContent = Array.from(
      { length: MAX_INLINE_DIFF_TOKENS + 1 },
      (_, index) => `a${index}`,
    ).join(" ");
  ...
  it("caps total inline diff pair work", () => {
    const oldContent = Array.from(
      { length: 260 },
      (_, index) => `const value${index} = oldName + ${index};`,
    ).join("\n");
    const newContent = Array.from(
      { length: 260 },
      (_, index) => `const value${index} = newName + ${index};`,
    ).join("\n");
