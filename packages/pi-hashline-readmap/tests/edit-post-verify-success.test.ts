...
describe("edit postEditVerify success", () => {
  ...
  it("reads back after a successful opt-in write and preserves the normal success details", async () => {
    ...
    fsMock.writeFile.mockImplementation(async (_path: string, content: string) => {
      persisted = content;
    });
    ...
    const result = await tool.execute(
      "tc",
      {
        path: "/tmp/post-edit-success.txt",
        postEditVerify: true,
    ...
    expect(fsMock.writeFile).toHaveBeenCalledWith(
      "/tmp/post-edit-success.txt",
      "ALPHA\nbeta",
      "utf-8",
    );
    expect(fsMock.readFile).toHaveBeenCalledTimes(2);
    expect(fsMock.readFile.mock.invocationCallOrder[1]).toBeGreaterThan(
      fsMock.writeFile.mock.invocationCallOrder[0],
    );
