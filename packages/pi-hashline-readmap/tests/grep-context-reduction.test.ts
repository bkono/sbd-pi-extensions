...
describe("grep context reduction", () => {
  ...
  it("parseGrepIR correctly counts matches and groups by file", () => {
    const lines = ["src/foo.ts:>>1:ab|hello", "src/foo.ts:>>5:cd|world", "src/bar.ts:>>3:ef|baz"];
  ...
  it("merges overlapping context windows — each line appears once", () => {
    ...
    expect(rawTexts.filter((r) => /(?:>>| {2})3:/.test(r)).length).toBe(1);
    expect(rawTexts.filter((r) => /(?:>>| {2})5:/.test(r)).length).toBe(1);
    expect(rawTexts.filter((r) => /(?:>>| {2})7:/.test(r)).length).toBe(1);

    const line5 = deduped.find((l) => /(?:>>| {2})5:/.test(l.raw));
    expect(line5?.kind).toBe("match");
    const line7 = deduped.find((l) => /(?:>>| {2})7:/.test(l.raw));
