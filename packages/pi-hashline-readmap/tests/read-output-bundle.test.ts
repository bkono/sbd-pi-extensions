...
describe("buildReadOutput bundle sections", () => {
  ...
  it("renders requested symbol and local support sections while adding ptcValue.bundle", () => {
    ...
    const requestedLines = sourceLines
      .slice(8, 12)
      .map((raw, index) => buildPtcLine(index + 9, raw));
    const helperOneLines = sourceLines
      .slice(0, 3)
      .map((raw, index) => buildPtcLine(index + 1, raw));
    const helperTwoLines = sourceLines
      .slice(4, 7)
      .map((raw, index) => buildPtcLine(index + 5, raw));
