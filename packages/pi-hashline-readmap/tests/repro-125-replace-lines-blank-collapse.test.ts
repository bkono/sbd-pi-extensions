...
describe("issue 125: replace_lines blank-line collapse in new_text", () => {
  ...
  it("preserves explicit blank paragraph separator in new_text", () => {
    const origContent = ["Some intro.", "    Previously, the system did X.", "End."].join("\n");
    ...
    const expected = ["Some intro.", "", "Previously, the system did X.", "End."].join("\n");

    expect(result.content).toBe(expected);
  });

  it("does not steal leading whitespace from replaced old line", () => {
    const origContent = ["Heading", "        Old line with lots of leading whitespace."].join("\n");
    ...
    const expected = ["Heading", "", "Old line with lots of leading whitespace."].join("\n");

    expect(result.content).toBe(expected);
  });

  it("multi-paragraph new_text with shared paragraph text keeps blank separators", () => {
    const origContent = ["A", "    Previously, things worked.", "B"].join("\n");

    const anchor = `2:${computeLineHash(2, "    Previously, things worked.")}`;
    const newText = ["Now, things work differently.", "", "Previously, things worked."].join("\n");
  ...
  it("keeps the blank separator in a mixed batch where another edit succeeds", () => {
    const origContent = ["const one = 1;", "    Previously, the system did X.", "Tail"].join("\n");
    ...
    expect(result.content).toBe(
      ["const one = 11;", "", "Previously, the system did X.", "Tail"].join("\n"),
    );
  });

  it("still restores a true soft wrap when no blank lines are involved", () => {
    const origContent = ["const summary = alpha + beta + gamma;", "tail();"].join("\n");
