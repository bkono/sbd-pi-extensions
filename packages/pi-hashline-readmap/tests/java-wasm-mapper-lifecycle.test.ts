...
describe("Java WASM mapper lifecycle", () => {
  it("uses the shared WASM loader and deletes parser and tree objects", async () => {
    ...
    const { javaMapperFromContent, MAPPER_VERSION } = await import(
      "../src/readmap/mappers/java.js"
    );
    ...
    const source = readFileSync(packagePath("src/readmap/mappers/java.ts"), "utf8");
