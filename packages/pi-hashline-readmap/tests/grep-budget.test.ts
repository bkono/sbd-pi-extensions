...
import { parsePositiveBase10Int, resolveGrepOutputBudget } from "../src/grep-budget.js";
import {
...
describe("resolveGrepOutputBudget", () => {
  ...
  it("uses JSON grep budget when env is unset", async () => {
    ...
    await writeFile(
      projectSettingsPath,
      JSON.stringify({ grep: { maxLines: 12, maxBytes: 1024 } }),
    );
    __setHashlineSettingsPathsForTest({
  ...
  it("clamps above-default JSON grep budget to the built-in ceilings", async () => {
    ...
    await writeFile(
      projectSettingsPath,
      JSON.stringify({ grep: { maxLines: 99999, maxBytes: 999999999 } }),
    );
    __setHashlineSettingsPathsForTest({
      globalSettingsPath: join(root, "missing.json"),
      projectSettingsPath,
  ...
  it.each([
    ...
    "1,000",
    "+5",
  ])("falls back to defaults for invalid value %j", (raw) => {
