...
describe("edit postEditVerify mismatch", () => {
  ...
  it("returns a structured error when persisted content differs after the write completed", async () => {
    ...
    const result = await tool.execute(
      "tc",
      {
        path: "/tmp/post-edit-mismatch.txt",
        postEditVerify: true,
    ...
    expect(result.content[0].text).toContain(
      "write completed but post-edit verification did not confirm the intended content",
    );
    ...
    expect(result.details.ptcValue.error.details).toEqual({
      expectedLength: "ALPHA\nbeta".length,
      actualLength: "tampered\nbeta".length,
    });
    ...
    expect(result.details.contextHygiene.resources).toEqual([
      {
        kind: "file",
        path: "/tmp/post-edit-mismatch.txt",
        key: "file:/tmp/post-edit-mismatch.txt",
      },
    ]);
  });
});
