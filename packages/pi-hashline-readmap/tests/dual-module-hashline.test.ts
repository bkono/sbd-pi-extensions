...
describe("hashline singleton state under dual module instances", () => {
  it("shares initialized hash state and supports the read -> grep -> write -> read -> edit flow", async () => {
    ...
    const base = `${packagePath("src")}/`;
    const extensionless = await jiti.import<typeof import("../src/hashline.js")>(`${base}hashline`);
    const withExtension = await jiti.import<typeof import("../src/hashline.js")>(
      `${base}hashline.js`,
    );
    ...
    expect(withExtension.computeLineHash(1, "hello")).toBe(
      extensionless.computeLineHash(1, "hello"),
    );

    const init = await jiti.import<any>(packagePath("index.ts"), { default: true });
