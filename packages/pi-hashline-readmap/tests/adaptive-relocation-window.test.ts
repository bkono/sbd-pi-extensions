...
describe("adaptive relocation window", () => {
  ...
  it("adaptive window is capped at 100", () => {
    ...
    const edits: any[] = [{ set_line: { anchor: `100:${hash100}`, new_text: "edited" } }];
    for (let i = 0; i < 24; i++) {
      const ln = i + 1;
      edits.push({
        set_line: {
          anchor: `${ln}:${computeLineHash(ln, lines[ln - 1])}`,
          new_text: lines[ln - 1],
        },
      });
    }
  ...
  it("mismatch error reports the actual window size", () => {
    ...
    // 5 edits → window = 25
    const edits: any[] = [{ set_line: { anchor: `30:${hash30}`, new_text: "edited" } }];
    for (let i = 0; i < 4; i++) {
      const ln = i + 1;
      edits.push({
        set_line: {
