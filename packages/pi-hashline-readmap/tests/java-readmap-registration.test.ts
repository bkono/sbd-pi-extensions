...
describe("Java readmap registration and workflow integration", () => {
  ...
  it("uses registered Java maps for read map rendering and symbol reads", async () => {
    ...
    expect(text(classRead)).toMatch(
      /^\[Symbol: KafkaConsumerConfiguration \(class\), lines 6-21 of 2\d\]/,
    );
    expect(text(classRead)).toContain("public class KafkaConsumerConfiguration");

    const methodRead = await readTool({
      path: fixture,
      symbol: "KafkaConsumerConfiguration.consumerFactory",
    });
    expect(text(methodRead)).toMatch(
      /^\[Symbol: consumerFactory \(method\) in KafkaConsumerConfiguration, lines 13-16 of 2\d\]/,
    );
    expect(text(methodRead)).toContain("return Map.of");
  });

  it("reads package-qualified Java inner type symbols", async () => {
    const innerRead = await readTool({
      path: qualifiedFixture,
      symbol: "com.example.qualified.QualifiedOuter.InnerType",
    });
  ...
  it("reads package-qualified Java top-level types", async () => {
    const outerRead = await readTool({
      path: qualifiedFixture,
      symbol: "com.example.qualified.QualifiedOuter",
    });
  ...
  it("uses registered Java maps for local bundles and symbol-scoped grep", async () => {
    const bundleRead = await readTool({
      path: fixture,
      symbol: "KafkaConsumerConfiguration.consumerFactory",
      bundle: "local",
    });
    ...
    const grep = await grepTool({
      pattern: "bootstrap.servers",
      path: fixture,
      literal: true,
      scope: "symbol",
    });
    expect(text(grep)).toContain(
      `--- ${basename(fixture)} :: method consumerFactory in KafkaConsumerConfiguration (13-16, 1 matches) ---`,
    );
