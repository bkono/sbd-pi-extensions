...
async function callSgTool(params: Record<string, unknown>, debugEnv?: string) {
  ...
  try {
    ...
    return await capturedTool.execute("test-call", params, new AbortController().signal, () => {}, {
      cwd: process.cwd(),
    });
  } finally {
...
describe("ast_search contextHygiene metadata", () => {
  ...
  it("findEnclosingSgSymbols derives real symbols from readmap metadata", async () => {
    ...
    await expect(
      findEnclosingSgSymbols(filePath, [{ startLine: 45, endLine: 49 }]),
    ).resolves.toEqual([{ name: "createDemoDirectory", kind: "function" }]);
  });
    ...
    vi.mocked(cp.execFile).mockImplementation((_cmd: any, _args: any, _opts: any, cb: any) => {
      cb(
        ...
        JSON.stringify([
          {
            file: filePath,
            range: { start: { line: 19, column: 0 }, end: { line: 32, column: 0 } },
          },
          {
            file: filePath,
            range: { start: { line: 34, column: 0 }, end: { line: 36, column: 0 } },
          },
        ]),
        "",
      );
