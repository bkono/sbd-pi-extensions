...
describe("Java WASM parser failure safety", () => {
  it("returns safely for malformed real input", async () => {
    const { javaMapperFromContent } = await import("../src/readmap/mappers/java.js");
    await expect(
      javaMapperFromContent("Bad.java", "public class Bad { void x( { \n".repeat(100)),
  ...
  it("swallows parser corruption exceptions and reports in debug mode", async () => {
    ...
    const { javaMapperFromContent } = await import("../src/readmap/mappers/java.js");
    await expect(javaMapperFromContent("Bad.java", "class Bad {}\n")).resolves.toBeNull();
