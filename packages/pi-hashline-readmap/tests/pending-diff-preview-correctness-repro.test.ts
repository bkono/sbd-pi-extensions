...
} from "../src/pending-diff-preview.js";

function expectOk<T extends { type: string; reason?: string }>(
  preview: T,
): asserts preview is T & { type: "ok" } {
  if (preview.type !== "ok")
    throw new Error(`preview skipped: ${preview.reason ?? "unknown reason"}`);
}
