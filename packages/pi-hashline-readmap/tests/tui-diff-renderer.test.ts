...
const diffData: DiffData = {
  version: 1,
  stats: { added: 2, removed: 1, context: 1 },
  entries: [
  ...
  inlineDiffs: [
    {
      removeLineIndex: 1,
      addLineIndex: 2,
      removeSpans: [{ kind: "remove", text: "two" }],
      addSpans: [{ kind: "add", text: "TWO" }],
    },
  ],
};
...
describe("renderTuiDiff", () => {
  it("renders unified, compact, split, and summary modes within width", () => {
    ...
    for (const rendered of [wide, normal, narrow, tiny])
      expect(rendered.lines.every((line) => visibleWidth(line) <= rendered.width)).toBe(true);
  });

  it("emits textual gutter markers without relying on color", () => {
    const normal = renderTuiDiff({
      diffData: markerDiffData,
      width: 80,
      theme: identityTheme,
      expanded: true,
    });
  ...
  it("same-line add and remove rows with identical text are distinct when color is stripped", () => {
    const normal = renderTuiDiff({
      diffData: identicalSameLineDiffData,
      width: 80,
      theme: identityTheme,
      expanded: true,
    });
  ...
  it("wraps long diff rows with hanging indent instead of truncating with ellipsis", () => {
    const longText =
      "the quick brown fox jumps over the lazy dog and keeps going far past the right edge";
    ...
    while (
      cursor < out.lines.length &&
      out.lines[cursor]!.startsWith(indent) &&
      !out.lines[cursor]!.startsWith("▌")
    ) {
