...
function capture(register: (pi: any) => any): any {
  let registered: any;
  register({
...
describe("list TUI renderers", () => {
  it("renders compact find output", () => {
    ...
    expect(textOf(find.renderCall({ pattern: "*.ts", path: "src" }, theme))).toBe(
      "find *.ts in src",
    );
    const result = {
      content: [{ type: "text", text: "a.ts\nb.ts" }],
      details: {
        ptcValue: { tool: "find", totalEntries: 2, truncated: false, entries: ["a.ts", "b.ts"] },
      },
    };
    expect(textOf(find.renderResult(result, {}, theme, {}))).toBe(
  ...
  it("wraps the find path title in an OSC 8 hyperlink when supported", () => {
    ...
    setCapabilities({ images: null, trueColor: true, hyperlinks: false });
    expect(textOf(find.renderCall({ pattern: "*.ts", path: "src" }, theme, { cwd }))).toBe(
      "find *.ts in src",
    );
  });
  ...
  it("renders compact ls output", () => {
    ...
    const result = {
      content: [{ type: "text", text: "read.ts\nwrite.ts" }],
      details: {
        ptcValue: {
          tool: "ls",
          totalEntries: 2,
          truncated: false,
          entries: ["read.ts", "write.ts"],
        },
      },
    };
    expect(textOf(ls.renderResult(result, {}, theme, {}))).toBe(
  ...
  it("keeps find and ls error summaries visible and expands details", () => {
    ...
    expect(
      textOf(find.renderResult(failed, { expanded: true }, theme, { expanded: true })),
    ).toContain("full detail");
    expect(textOf(ls.renderResult(failed, {}, theme, {}))).toBe("↳ permission denied");
    expect(
      textOf(ls.renderResult(failed, { expanded: true }, theme, { expanded: true })),
    ).toContain("full detail");
  });
});
