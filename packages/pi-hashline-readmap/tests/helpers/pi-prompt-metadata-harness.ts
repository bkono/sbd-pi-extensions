...
export async function collectHashlineSystemPromptMetadata(
  activeTools: string[],
): Promise<PromptMetadataResult> {
  ...
  try {
    ...
    for (const toolName of activeTools) {
      const definition = session.getToolDefinition(toolName) as
        | (ToolDefinition & {
      ...
      if (guidelines.length === 0)
        throw new Error(`Expected ${toolName} to expose non-empty promptGuidelines`);
