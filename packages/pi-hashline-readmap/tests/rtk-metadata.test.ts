...
describe("filterBashOutput info (router technique tagging)", () => {
  ...
  it.each(cases)("tags info.technique === '$technique' when that route fires", ({
    technique,
    command,
    mock,
  }) => {
...
describe("RTK notice format", () => {
  it("renders KB with one decimal place and rounded percentage", async () => {
    ...
    try {
      ...
      expect(
        text.startsWith(
          "[RTK: compressed git output 8.2 KB → 1.2 KB (86% saved). Use `PI_RTK_BYPASS=1 git diff` to see full output.]\n",
        ),
      ).toBe(true);
    } finally {
      spy.mockRestore();
