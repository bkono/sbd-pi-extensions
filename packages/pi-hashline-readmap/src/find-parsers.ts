...
export function parseSize(field: string, value: number | string): number {
  if (typeof value === "number") {
    if (!Number.isFinite(value) || value < 0) {
      throw new Error(`Invalid ${field} value: ${value} (expected a non-negative number of bytes)`);
    }
    return value;
  }
...
export function parseRelativeOrIsoDate(field: string, value: string, now: Date = new Date()): Date {
