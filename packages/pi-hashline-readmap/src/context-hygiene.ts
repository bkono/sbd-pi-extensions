...
export function buildStaleContextRecord(
  input: BuildStaleContextRecordInput,
  ...
  if (input.originalEventId !== undefined) record.originalEventId = input.originalEventId;
  if (input.originalResultId) record.originalResultId = input.originalResultId;
  if (input.invalidatingMutationResultId)
    record.invalidatingMutationResultId = input.invalidatingMutationResultId;
...
export function classifyCommandForContextHygiene(command: string): ContextHygieneCommandKind {
  ...
  if (/\b(install|ci|add)\b/.test(normalized) && /^(npm|pnpm|yarn|bun)\b/.test(normalized))
    return "install";
...
export interface ContextHygieneTracker {
  record(
    ...
    options?: ContextHygieneRecordOptions,
  ): ContextHygieneEvent;
...
export function registerContextHygieneDebugTool(
  ...
  const tool = {
    ...
    ptc: typeof CONTEXT_HYGIENE_DEBUG_TOOL_PTC;
  };
...
class DefaultContextHygieneTracker implements ContextHygieneTracker {
  ...
  constructor(options: CreateContextHygieneTrackerOptions = {}) {
    this.maxEvents = Math.max(
      1,
      Math.floor(options.maxEvents ?? DEFAULT_CONTEXT_HYGIENE_MAX_EVENTS),
    );
  }
  ...
  record(
    ...
    if (metadata.rehydrate)
      event.rehydrate = cloneContextHygieneRehydrateDescriptor(metadata.rehydrate);
  ...
  generateReport(): ContextHygieneReport {
    ...
    const commandReruns = sortResourceKeys(commandEventsByResource.keys()).flatMap(
      (resourceKey) => {
    ...
    for (const resourceKey of sortResourceKeys(mutationEventsByResource.keys())) {
      ...
      for (const mutation of mutations) {
        ...
        mutationAfterRead.push({
          resourceKey,
          readEventIds: priorReadIds,
        ...
        staleCandidates.push({
          ...
          staleResults: priorReads.map((read) =>
    ...
    const bashCommandEvents = this.events.filter(
      (event) =>
        event.tool === "bash" && event.classification === "command-output" && event.commandState,
    );
    const invalidatingRepoEvents = this.events.filter(
      (event) =>
        event.classification === "mutation" ||
        event.commandState?.stateKind === "git-worktree-mutation",
    );
    ...
    for (const event of bashCommandEvents) {
      ...
      if (!state || (state.stateKind !== "repo-status" && state.stateKind !== "repo-diff"))
        continue;
    ...
    for (const event of bashCommandEvents) {
      ...
      const success = bashCommandEvents.find((candidate) => {
        ...
        return (
          ...
          commandKeyForEvent(candidate) === commandKey &&
    ...
    for (const resourceKey of sortResourceKeys(commandEventsByResource.keys())) {
      const eligible = (commandEventsByResource.get(resourceKey) ?? []).filter(
        (event) =>
          event.commandState?.routineRetirementEligible === true &&
          event.commandState.outcome === "success",
      );
      ...
      retirementCandidates.push({
        ...
        retiredResults: retired.map((event) =>
    ...
    return {
      ...
      churn: {
        ...
        byTool: Object.fromEntries(
          Object.entries(byTool).sort(([left], [right]) => compareStable(left, right)),
        ),
...
export function createContextHygieneTracker(
  options: CreateContextHygieneTrackerOptions = {},
...
export function resetContextHygieneTracker(
  options: CreateContextHygieneTrackerOptions = {},
): ContextHygieneTracker {
