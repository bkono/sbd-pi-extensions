...
import { generateCompactOrFullDiff, hasBareCarriageReturn, normalizeToLF } from "./edit-diff.js";
...
import {
  ...
  buildWritePreviewKey,
  type PendingDiffPreviewResult,
...
function formatContentPreviewLines(content: string, theme: any): string[] {
  ...
  const fg =
    typeof theme?.fg === "function"
      ? (style: string, text: string) => theme.fg(style, text)
  ...
  if (lines.length > CONTENT_PREVIEW_MAX_LINES) {
    formatted.push(
      `  ${fg("dim", `… ${lines.length - CONTENT_PREVIEW_MAX_LINES} more lines not shown`)}`,
    );
  }
  return formatted;
}

function pendingWritePreviewParts(
  summary: string,
  preview: PendingDiffPreviewResult | undefined,
  expanded: boolean,
  theme: any,
): { lines: string[]; diffData?: DiffData } {
...
export async function executeWrite(opts: {
  ...
  if (hasBareCarriageReturn(content)) {
    const message =
      "File content contains bare CR (\\r) line endings; write refuses to emit anchors that read/edit would normalize differently.";
...
export function registerWriteTool(pi: ExtensionAPI, options: WriteToolOptions = {}) {
  ...
  const tool = {
    ...
    async execute(
      _toolCallId: string,
      params: { path: string; content: string; map?: boolean },
    ...
    renderCall(args: any, theme: any, context: any = {}) {
      ...
      const renderedPath =
        typeof path === "string"
          ...
          : theme.fg("toolOutput", "...");
      const text = clampLineToWidth(
        `${label} ${renderedPath}${typeof content === "string" ? ` (${lineCount} ${lineCount === 1 ? "line" : "lines"} • ${bytes} B)` : ""}`,
        context.width,
      );
      ...
      if (context.executionStarted) {
        const textComponent =
          context.lastComponent && !(context.lastComponent instanceof DiffPreviewComponent)
      ...
      if (parts.diffData) {
        const diffComponent =
          context.lastComponent instanceof DiffPreviewComponent
            ? context.lastComponent
            : new DiffPreviewComponent({
                prefixLines: parts.lines,
                diffData: parts.diffData,
                theme,
                expanded: true,
              });
        diffComponent.update({
          prefixLines: parts.lines,
          diffData: parts.diffData,
          theme,
          expanded: true,
    ...
    renderResult(result: any, options: any, theme: any, context: any = {}) {
      ...
      const text = summaryLine(state, { hidden: hasExpandableDiff && !expanded });
      if (expanded && hasExpandableDiff) {
        const diffComponent =
          context.lastComponent instanceof DiffPreviewComponent
            ? context.lastComponent
            : new DiffPreviewComponent({
                prefixLines: text.split("\n"),
                diffData,
                theme,
                expanded: true,
              });
