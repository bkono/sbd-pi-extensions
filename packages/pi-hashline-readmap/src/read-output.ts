...
import { buildPtcLines, type PtcLine, type PtcWarning, renderPtcLines } from "./ptc-value.js";
...
export function buildReadOutput(input: ReadOutputInput): ReadOutputResult {
  ...
  if (input.symbol) {
    contextHygieneResources.push(
      buildSymbolResource(input.path, input.symbol.name, input.symbol.kind),
    );
  }
  if (input.bundle?.applied) {
    for (const support of input.bundle.localSupport) {
      contextHygieneResources.push(
        buildSymbolResource(input.path, support.symbol.name, support.symbol.kind),
      );
    }
  }
