...
function buildFallbackEntries(
  ...
  for (const matchLine of matchLines) {
    ...
    for (let lineNumber = start; lineNumber <= end; lineNumber++) {
      const built = buildPtcLine(lineNumber, fileLines[lineNumber - 1] ?? "");
      const candidate: GrepOutputEntry = {
        kind: lineNumber === matchLine ? "match" : "context",
        line: built,
      };
...
export function scopeGrepGroupsToSymbols(input: {
  ...
  for (const group of input.groups) {
    ...
    const symbolBuckets = new Map<
      string,
      { symbol: GrepOutputScopeSymbol; matchLines: Set<number> }
    >();
