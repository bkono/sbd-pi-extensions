...
function isFileSymbolLike(value: unknown): boolean {
  ...
  if (
    ...
    (!Array.isArray(value.children) || !value.children.every(isFileSymbolLike))
  ) {
    return false;
  }
...
export function __setEvictionHooksForTest(hooks: { interval?: number; cap?: number } | null): void {
  if (hooks === null) {
