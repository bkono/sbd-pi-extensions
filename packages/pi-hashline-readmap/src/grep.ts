  ...
  const tool = {
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (scopeContext.value !== undefined && rawParams.scope !== "symbol") {
        const message =
          'Invalid scopeContext: requires scope: "symbol". For normal surrounding-line context outside symbol scope, use the `context` parameter.';
        return {
          content: [
            {
              type: "text",
      ...
      const existingDetails =
        typeof result.details === "object" && result.details !== null
          ...
          : {};
      const {
        linesTruncated: _ignoredLinesTruncated,
        truncation: _ignoredTruncation,
        ...compactDetails
      } = existingDetails;
      return {
        ...result,
