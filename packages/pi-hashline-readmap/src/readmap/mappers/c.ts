...
const C_PATTERNS = {
  ...
  variable: /^(?:static\s+)?(?:const\s+)?(?:volatile\s+)?(\w+(?:\s*\*)?)\s+(\w+)\s*(?:=|;)/,
 ...
 */
export async function cMapper(filePath: string, _signal?: AbortSignal): Promise<FileMap | null> {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (funcMatch) {
        ...
        if (!trimmed.includes("(") || !name || /^(if|while|for|switch|return)$/.test(name)) {
          continue;
        }
    ...
    const symbols: FileSymbol[] = matches.map((m) => {
      const { startLine } = m;
      const endLine =
        m.kind === SymbolKind.Constant ? startLine : findBlockEnd(lines, startLine - 1);
