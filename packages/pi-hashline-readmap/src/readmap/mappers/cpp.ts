...
function formatSignature(node: SyntaxNode, source: string, templatePrefix?: string): string {
...
function findDescendantOfType(node: SyntaxNode, type: string): SyntaxNode | null {
...
function findFirstDescendant(node: SyntaxNode, types: string[]): SyntaxNode | null {
...
function collectDescendants(node: SyntaxNode, type: string): SyntaxNode[] {
...
function extractTypedefName(node: SyntaxNode, source: string): string | null {
...
function extractAliasName(node: SyntaxNode, source: string): string | null {
  const nameNode =
    node.childForFieldName("name") ?? findFirstDescendant(node, ["type_identifier", "identifier"]);
  return nameNode ? normalizeWhitespace(getNodeText(nameNode, source)) : null;
}

function extractSpecifierName(node: SyntaxNode, source: string, pattern: RegExp): string | null {
  const nameNode =
    node.childForFieldName("name") ?? findFirstDescendant(node, ["type_identifier", "identifier"]);
  if (nameNode) {
...
function collectFieldNames(node: SyntaxNode, source: string): string[] {
 ...
 * Handles /// and /** styles.
 */
function extractDocComment(node: SyntaxNode, source: string): string | undefined {
...
function extractCppSymbols(root: SyntaxNode, content: string): InternalSymbol[] {
  ...
  const handleNamespace = (node: SyntaxNode, templatePrefix?: string): void => {
  ...
  const handleClass = (
    ...
    const signature = formatSignature(node, content, templatePrefix);
    const name =
      extractSpecifierName(node, content, new RegExp(`\\b${kind}\\s+([A-Za-z_][\\w:]*)`)) ??
    ...
    const body =
      ...
      node.namedChildren.find(
        (child) => !!child && ["field_declaration_list", "declaration_list"].includes(child.type),
      ) ??
      null;
    const children = body ? body.namedChildren : node.namedChildren;
    for (const child of children) {
  ...
  const handleEnum = (node: SyntaxNode, templatePrefix?: string): void => {
    const signature = formatSignature(node, content, templatePrefix);
    const name =
      extractSpecifierName(node, content, /\benum(?:\s+(?:class|struct))?\s+([A-Za-z_][\w:]*)/) ??
  ...
  const handleTypedef = (node: SyntaxNode, templatePrefix?: string): void => {
    const signature = formatSignature(node, content, templatePrefix);
    const name =
  ...
  const handleAlias = (node: SyntaxNode, templatePrefix?: string): void => {
    const signature = formatSignature(node, content, templatePrefix);
  ...
  const handleFunction = (node: SyntaxNode, templatePrefix?: string): void => {
    const signature = formatSignature(node, content, templatePrefix);
    ...
    if (qualifiedParent && scope?.kind === "namespace" && !qualifiedParent.includes("::")) {
  ...
  const visit = (node: SyntaxNode, templatePrefix?: string): void => {
    switch (node.type) {
      case "template_declaration": {
        ...
        if (decl) {
          const prefix = normalizeWhitespace(content.slice(node.startIndex, decl.startIndex));
 ...
 */
export async function cppMapper(filePath: string, signal?: AbortSignal): Promise<FileMap | null> {
