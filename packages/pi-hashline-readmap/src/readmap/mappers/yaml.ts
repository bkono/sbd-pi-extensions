 ...
 */
function convertKeysToSymbols(keys: YamlKey[], totalLines: number): FileSymbol[] {
  if (keys.length === 0) {
    return [];
  }
 ...
 */
export async function yamlMapper(filePath: string, signal?: AbortSignal): Promise<FileMap | null> {
  try {
