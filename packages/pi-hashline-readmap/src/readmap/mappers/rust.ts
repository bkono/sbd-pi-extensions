...
function extractNameFromField(node: SyntaxNode, field: string, source: string): string | null {
...
function normalizeTypePath(path: string, moduleKey: string | null): string {
  ...
  if (superCount > 0) {
    const trimmed = moduleParts.slice(0, Math.max(0, moduleParts.length - superCount));
...
function extractRustSymbols(root: SyntaxNode, content: string): InternalSymbol[] {
  ...
  const handleFunction = (node: SyntaxNode): void => {
    ...
    const moduleKey = getModuleKey(scopeStack);
    const parentName = implScope?.implTarget ?? traitScope?.key ?? moduleKey ?? undefined;
 ...
 */
export async function rustMapper(filePath: string, signal?: AbortSignal): Promise<FileMap | null> {
  const parser = await getWasmParser("rust");
  if (!parser) return null;
  let tree: Tree | null = null;
  try {
