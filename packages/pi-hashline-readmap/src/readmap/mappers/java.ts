...
import { getLineRange, getNodeText, normalizeWhitespace } from "./tree-sitter-helpers.js";
...
function formatSignature(
  node: SyntaxNode,
  source: string,
  options?: { stripLeadingAnnotations?: boolean },
): string {
...
function variableDeclarators(node: SyntaxNode): SyntaxNode[] {
  return node.namedChildren.filter(
    (child): child is SyntaxNode => !!child && child.type === "variable_declarator",
  );
}

function pushSymbol(
  parent: FileSymbol | undefined,
  rootSymbols: FileSymbol[],
  symbol: FileSymbol,
): void {
  if (parent) {
    parent.children ??= [];
...
function handleVariableDeclaration(
  ...
  const kind =
    ...
    (modifiers.includes("static") && modifiers.includes("final"))
      ? SymbolKind.Constant
      : SymbolKind.Property;
  ...
  for (const declarator of variableDeclarators(node)) {
    ...
    pushSymbol(parent, rootSymbols, symbol);
  }
}
...
function extractSymbols(root: SyntaxNode, source: string): FileSymbol[] {
  ...
  const visit = (node: SyntaxNode, parent?: FileSymbol): void => {
    ...
    if (typeKind) {
      ...
      if (!name) return;
      const symbol = buildSymbol(node, source, name, typeKind, {
        stripLeadingAnnotations: Boolean(parent),
      });
      pushSymbol(parent, rootSymbols, symbol);
      const body = node.childForFieldName("body");
    ...
    if (MEMBER_METHOD_TYPES.has(node.type)) {
      const name = extractName(node, source);
      if (!name) return;
      pushSymbol(parent, rootSymbols, buildSymbol(node, source, name, SymbolKind.Method));
      return;
    }

    if (node.type === "static_initializer") {
      pushSymbol(parent, rootSymbols, buildSymbol(node, source, "<clinit>", SymbolKind.Method));
      return;
    }
