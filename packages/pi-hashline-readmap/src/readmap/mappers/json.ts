...
function schemaToSymbols(
  ...
  for (const [key, value] of Object.entries(schema)) {
    ...
    if (typeof value === "string") {
      ...
      if (key === "[]") {
        // Array element
        const { symbols: childSymbols, lineEstimate: newLine } = schemaToSymbols(
          value as JsonSchema,
          key,
          lineEstimate,
        );
 ...
 */
export async function jsonMapper(filePath: string, signal?: AbortSignal): Promise<FileMap | null> {
  try {
    // Check if jq is available
    if (!(await hasJq())) {
