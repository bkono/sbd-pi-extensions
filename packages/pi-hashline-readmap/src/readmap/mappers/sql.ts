...
const SQL_PATTERNS = [
  ...
  // CREATE INDEX name ON table
  {
    ...
    prefix: "INDEX",
    formatName: (match: RegExpExecArray) => `${match[1]} ON ${match[2]}` as const,
  },
  ...
  // CREATE SCHEMA name
  {
    regex: /^\s*CREATE\s+SCHEMA\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:AUTHORIZATION\s+)?"?(\w+)"?/gim,
    kind: SymbolKind.Class,
    prefix: "SCHEMA",
  },
  // ALTER TABLE name
  {
    regex: /^\s*ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(?:ONLY\s+)?(?:"?(\w+)"?\.)?"?(\w+)"?/gim,
    kind: SymbolKind.Variable,
    prefix: "ALTER TABLE",
  },
];
 ...
 */
export async function sqlMapper(filePath: string, _signal?: AbortSignal): Promise<FileMap | null> {
  try {
