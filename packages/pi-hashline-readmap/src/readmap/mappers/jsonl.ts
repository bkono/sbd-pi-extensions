 ...
 */
function analyzeJsonLine(line: string): { keys: string[]; type: string } | null {
  try {
 ...
 */
export async function jsonlMapper(filePath: string, signal?: AbortSignal): Promise<FileMap | null> {
  try {
 ...
 */
async function readFirstLine(filePath: string, signal?: AbortSignal): Promise<string | null> {
  const stream = createReadStream(filePath, { encoding: "utf8" });
  const rl = createInterface({ input: stream });
