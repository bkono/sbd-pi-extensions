...
function getFunctionSignature(
  ...
  const generatorPrefix = Node.isFunctionDeclaration(node) && node.isGenerator() ? "*" : "";
...
function getTypeSignature(node: import("ts-morph").TypeAliasDeclaration): string {
  ...
  const maxLen = 200;
  const truncated = typeText.length > maxLen ? `${typeText.slice(0, maxLen)}...` : typeText;
...
function getPropertySignature(node: import("ts-morph").PropertyDeclaration): string {
...
function isVariableExported(varDecl: import("ts-morph").VariableDeclaration): boolean {
  const statement = varDecl.getVariableStatement();
  return statement?.isExported() ?? false;
}

function isVariableDefaultExport(varDecl: import("ts-morph").VariableDeclaration): boolean {
  const statement = varDecl.getVariableStatement();
 ...
 */
function getDocstring(node: import("ts-morph").JSDocableNode): string | undefined {
