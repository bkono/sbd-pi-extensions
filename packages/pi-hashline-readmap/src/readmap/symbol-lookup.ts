...
function preferJavaTopLevelType(
  map: FileMap,
...
export function findSymbol(map: FileMap, query: string): SymbolLookupResult {
  ...
  if (atLineMatch) {
    ...
    if (namePart.includes(".")) {
      ...
      pool = parts.every((p) => p.length > 0) ? resolveDotPath(map.symbols, parts) : [];
    } else {
    ...
    if (pool.length === 0) {
      ...
      const sameLeaf = allSymbols.filter(
        (c) =>
          c.symbol.name === leaf && Number.isFinite(c.symbol.startLine) && c.symbol.startLine > 0,
      );
  ...
  if (exact.length === 1)
    return { type: "found", symbol: toMatch(exact[0].symbol, exact[0].parentName) };
  if (exact.length > 1) {
    const preferred = preferJavaTopLevelType(map, exact);
    if (preferred)
      return { type: "found", symbol: toMatch(preferred.symbol, preferred.parentName) };
  ...
  if (javaRelativeQuery) {
    ...
    if (javaExact.length === 1)
      return { type: "found", symbol: toMatch(javaExact[0].symbol, javaExact[0].parentName) };
    if (javaExact.length > 1) {
    ...
    if (parts.length >= 2 && parts.every((p) => p.length > 0)) {
      ...
      if (javaParts.length >= 2 && javaParts.every((p) => p.length > 0)) {
        ...
        if (javaCandidates.length === 1) {
          return {
            type: "found",
            symbol: toMatch(javaCandidates[0].symbol, javaCandidates[0].parentName),
          };
        }
        if (javaCandidates.length > 1) {
          return {
  ...
  if (prefix.length === 1)
    return { type: "found", symbol: toMatch(prefix[0].symbol, prefix[0].parentName) };
  ...
  if (camelCase.length > 1)
    return { type: "ambiguous", candidates: toMatches(camelCase.slice(0, 5)) };
  if (partial.length === 1) {
    return {
      type: "fuzzy",
