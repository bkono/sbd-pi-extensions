 ...
 */
function formatSymbol(symbol: FileSymbol, level: DetailLevel, indent = 0): string {
 ...
 */
function formatSymbols(symbols: FileSymbol[], level: DetailLevel, indent = 0): string[] {
  const lines: string[] = [];
...
export function formatFileMap(map: FileMap, level?: DetailLevel): string {
  ...
  if (map.truncatedInfo) {
    ...
    lines.push(`[Map ≤${formatSize(THRESHOLDS.COMPACT_TARGET_BYTES)} | compact]`);
    lines.push("");
  }
  ...
  if (map.truncatedInfo) {
    ...
    // Add separator
    lines.push("");
    lines.push(`  ─ ─ ─ ${formatNumber(map.truncatedInfo.omittedSymbols)} more symbols ─ ─ ─`);
    lines.push("");
  ...
  if (map.truncatedInfo) {
    ...
    lines.push("Use read(path, offset=LINE, limit=N) to view specific sections.");
  } else {
