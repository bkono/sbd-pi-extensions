import { GENERIC_SUGGESTION, SUGGESTIONS } from "./doom-loop-suggestions.js";
...
function sameFingerprints(left: RecordedToolCall[], right: RecordedToolCall[]): boolean {
  return (
    ...
    left.every((call, index) => call.fingerprint === right[index]?.fingerprint)
  );
}
...
function hasIdenticalTail(calls: RecordedToolCall[]): boolean {
  ...
  return (
    calls[calls.length - 2]?.fingerprint === last && calls[calls.length - 3]?.fingerprint === last
  );
}
