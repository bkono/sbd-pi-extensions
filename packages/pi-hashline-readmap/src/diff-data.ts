...
function parseCompactDiffEntries(
  diff: string,
  oldContent: string,
  newContent: string,
): DiffEntry[] | undefined {
  ...
  let separator = separatorPattern.exec(diff);
  while (separator !== null) {
    ...
    separator = separatorPattern.exec(diff);
  }
  return undefined;
}
...
function buildInlineSpans(
  ...
  if (
    removeText.length > MAX_INLINE_DIFF_LINE_LENGTH ||
    addText.length > MAX_INLINE_DIFF_LINE_LENGTH
  )
    return undefined;
  ...
  const similarity =
    meaningfulEqualTokenCount / Math.max(meaningfulRemoveTokenCount, meaningfulAddTokenCount);
  ...
  for (const [removeIndex, addIndex] of pairs) {
    if (removeCursor < removeIndex) {
      pushMergedSpan(removeSpans, {
        kind: "remove",
        text: removeTokens.slice(removeCursor, removeIndex).join(""),
      });
    }
    if (addCursor < addIndex) {
      pushMergedSpan(addSpans, {
        kind: "add",
        text: addTokens.slice(addCursor, addIndex).join(""),
      });
    }
  ...
  if (removeCursor < removeTokens.length) {
    pushMergedSpan(removeSpans, {
      kind: "remove",
      text: removeTokens.slice(removeCursor).join(""),
    });
  }
  ...
  if (
    !removeSpans.some((span) => span.kind === "remove") ||
    !addSpans.some((span) => span.kind === "add")
  )
    return undefined;
...
function finalizeDiffData(
  path: string,
  entries: DiffEntry[],
  blockRanges: DiffBlockRange[] | undefined,
): DiffData {
...
export function buildDiffData(input: BuildDiffDataInput): DiffData {
  const entries =
    parseCompactDiffEntries(input.diff, input.oldContent, input.newContent) ??
    parseFullDiffEntries(input.diff);
