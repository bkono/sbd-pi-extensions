...
export function __setHashlineSettingsPathsForTest(paths: {
...
function defaultGlobalSettingsPath(): string {
  return join(homedir(), ".pi/agent/hashline-readmap/settings.json");
}
function defaultProjectSettingsPath(): string {
  return join(process.cwd(), ".pi/hashline-readmap/settings.json");
}
function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
function invalid(source: string, path: string): HashlineSettingsWarning {
...
function isStrictJsonPositiveInteger(
  rawText: string,
  path: string,
  value: unknown,
): value is number {
...
function validateSettings(raw: unknown, source: string, rawText: string): HashlineSettingsResult {
  ...
  if (isRecord(raw.mapCache)) {
    ...
    if ("dir" in raw.mapCache) {
      if (typeof raw.mapCache.dir === "string" && raw.mapCache.dir.length > 0)
        mapCache.dir = raw.mapCache.dir;
  ...
  if (isRecord(raw.bashContextGuard)) {
    ...
    for (const key of ["maxLines", "maxBytes", "headLines", "tailLines"] as const) {
      const value = readPositive(
        raw.bashContextGuard,
        key,
        `bashContextGuard.${key}`,
        source,
        rawText,
        warnings,
      );
...
function mergeSettings(
  base: HashlineJsonSettings,
  override: HashlineJsonSettings,
): HashlineJsonSettings {
  ...
  const bashContextGuard = {
    ...(base.bashContextGuard ?? {}),
    ...(override.bashContextGuard ?? {}),
  };
...
export function resolveHashlineJsonSettings(): HashlineSettingsResult {
  const globalResult = readSettingsFile(
    pathOverride?.globalSettingsPath ?? defaultGlobalSettingsPath(),
  );
  const projectResult = readSettingsFile(
    pathOverride?.projectSettingsPath ?? defaultProjectSettingsPath(),
  );
  return {
