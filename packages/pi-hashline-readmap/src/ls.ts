...
export function registerLsTool(pi: ExtensionAPI) {
  const tool: Parameters<ExtensionAPI["registerTool"]>[0] & { ptc: typeof LS_PTC } = {
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(
        Type.Union([Type.Number(), Type.String()], { description: "Max entries" }),
      ),
    ...
    renderCall(args: any, theme: any, context: any = {}) {
      ...
      return new Text(
        clampLineToWidth(`${renderToolLabel(theme, "ls")} ${linkedPath}`, context.width),
        0,
        0,
      );
    },
    ...
    renderResult(result: any, options: any, theme: any, context: any = {}) {
      ...
      let text = summaryLine(`${total} ${total === 1 ? "entry" : "entries"} returned`, {
        hidden: !!output && !expanded,
      });
