...
function pendingPreviewLines(
  summary: string,
  preview: PendingDiffPreviewResult | undefined,
  expanded: boolean,
): { lines: string[]; diffData?: ReturnType<typeof buildDiffData>; headerLabel?: string } {
  ...
  return {
    lines: [summary, headerLine],
    diffData: expanded ? diffData : undefined,
    headerLabel: preview.data.headerLabel,
  };
}
...
export function registerEditTool(pi: ExtensionAPI, options: EditToolOptions = {}) {
  ...
  const tool = {
      ...
      try {
        return await withFileMutationQueue(absolutePath, async () => {
          ...
          const replaceSymbolEdits = edits.filter(
            (e): e is { replace_symbol: { symbol: string; new_body: string } } =>
              "replace_symbol" in e,
          );
          ...
          const sortedReplaceSymbolRanges = [...replaceSymbolRanges].sort(
        ...
        if (typeof code === "string") {
          ...
          return buildEditError(absolutePath, "fs-error", message, undefined, {
            fsCode: code,
            fsMessage: err?.message,
          });
        }
        ...
        const diffComponent =
          context.lastComponent instanceof DiffPreviewComponent
            ...
            : new DiffPreviewComponent({
                prefixLines: preview2.lines,
                diffData: preview2.diffData,
                theme,
                expanded: true,
              });
    ...
    renderResult(result: any, options: ToolRenderResultOptions, theme: any, ...rest: any[]) {
      ...
      const semanticClassification = (ptcValue as any)?.semanticSummary?.classification as
        | string
        | undefined;
      ...
      const expanded =
        isRendererExpanded(options as any, context as any) ||
        resolveEditDiffDisplay() === "expanded";
      const width = (context as any).width ?? (options as any)?.width;
