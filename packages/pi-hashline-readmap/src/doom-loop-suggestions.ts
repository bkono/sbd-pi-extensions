...
export const SUGGESTIONS: Record<string, readonly string[]> = {
  ...
  find: ["try a looser glob", 'try type: "any"', "try a different path"],
  ls: ["try a different path", "remove the glob filter"],
};

export const GENERIC_SUGGESTION =
  "try a different approach — the repeating call is not making progress";
