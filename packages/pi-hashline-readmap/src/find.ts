...
async function findWithFd(
  ...
  return new Promise((resolve_, reject) => {
    ...
    execFile(
      "fd",
      args,
      { maxBuffer: 10 * 1024 * 1024, cwd: searchPath },
      (err, stdout, _stderr) => {
        if (err && !stdout) {
...
export function registerFindTool(pi: ExtensionAPI) {
  const tool: Parameters<ExtensionAPI["registerTool"]>[0] & { ptc: typeof FIND_PTC } = {
    ...
    renderResult(result: any, options: any, _theme: any, context: any = {}) {
      ...
      const total =
        ptcValue?.totalEntries ??
        output
          .split("\n")
          .filter((l: string) => l.length > 0 && !l.startsWith("[") && !l.startsWith("Hint"))
          .length;
      if (total === 0) return new Text(summaryLine("no results"), 0, 0);
      let text = summaryLine(`${total} ${total === 1 ? "result" : "results"} returned`, {
