...
function extractNewTextValues(edits: unknown[] | undefined): string[] {
  ...
  for (const edit of edits ?? []) {
    ...
    if ("set_line" in edit && typeof (edit as any).set_line?.new_text === "string")
      values.push((edit as any).set_line.new_text);
    if ("replace_lines" in edit && typeof (edit as any).replace_lines?.new_text === "string")
      values.push((edit as any).replace_lines.new_text);
    if ("insert_after" in edit && typeof (edit as any).insert_after?.new_text === "string")
      values.push((edit as any).insert_after.new_text);
    if ("replace" in edit && typeof (edit as any).replace?.new_text === "string")
      values.push((edit as any).replace.new_text);
  }
  return values;
}
function formatWhitespaceOnlyWarning(
