...
export async function findEnclosingSgSymbols(
  absPath: string,
  ranges: SgRange[],
): Promise<SgEnclosingSymbol[]> {
  ...
  for (const range of ranges) {
    const enclosing = allSymbols
      ...
      .sort(
        (a, b) => a.endLine - a.startLine - (b.endLine - b.startLine) || a.startLine - b.startLine,
      )[0];
...
export function isSgAvailable(): boolean {
  try {
    ...
    cp.execFileSync(binary.command, [...binary.argsPrefix, "--version"], {
      timeout: 3000,
      stdio: "pipe",
    });
    return true;
  ...
  const tool = {
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      try {
        ...
        const searchPathIsDirectory = await fsStat(searchPath)
        ...
        const enrichContextHygieneSymbols =
          isContextHygieneDebugEnabled() && grouped.size <= CONTEXT_HYGIENE_SG_SYMBOL_FILE_CAP;
        for (const [display, { abs, matches: fileMatches }] of grouped) {
          ...
          ptcFile.symbols = enrichContextHygieneSymbols
            ? await findEnclosingSgSymbols(abs, ranges)
            : [];
    ...
    renderResult(result: any, options: ToolRenderResultOptions, theme: any, ...rest: any[]) {
      ...
      if (isPartial)
        return new Text(clampLinesToWidth([summaryLine("pending search")], width).join("\n"), 0, 0);
      ...
      let text = summaryLine(`${totalMatches} ${matchWord} in ${fileCount} ${fileWord}`, {
