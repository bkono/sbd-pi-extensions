...
export function formatGrepCallText(args: Record<string, unknown> | undefined): GrepCallTextResult {
...
export function formatGrepResultText(input: GrepResultTextInput): GrepResultTextOutput {
  ...
  const summaryText = noMatches
    ? ""
    : `\u2713 ${totalMatches} ${matchWord} in ${fileCount} ${fileWord}`;
