...
import { hasBareCarriageReturn, normalizeToLF, stripBom } from "./edit-diff.js";
import { ensureHashInit, formatHashlineDisplay } from "./hashline.js";
import { getOrGenerateMap } from "./map-cache.js";
import { resolveToCwd } from "./path-utils.js";
import { buildPtcError, buildPtcLines, buildPtcWarning, type PtcWarning } from "./ptc-value.js";
...
const READ_PROMPT_METADATA = defineToolPromptMetadata({
  ...
  promptSnippet:
    "Read text files or images; text reads include hashline anchors and optional maps/symbol lookup",
  promptGuidelines: [
  ...
  const tool = {
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      if (p.bundle && !p.symbol) {
        const message =
          'Cannot use bundle without symbol. Use read({ path, symbol, bundle: "local" }).';
        return {
          content: [{ type: "text", text: message }],
          isError: true,
          details: {
      ...
      const shouldAppendMap =
        !!p.map || (!!truncation.truncated && !p.offset && !p.limit && !symbolMatch);
    ...
    renderResult(result: any, options: ToolRenderResultOptions, _theme: any, ...rest: any[]) {
      ...
      if (isPartial)
        return new Text(clampLinesToWidth([summaryLine("pending read")], width).join("\n"), 0, 0);
      ...
      const visibleLines =
        info.truncated && ptcValue.truncation
          ? ptcValue.truncation.outputLines
          : ptcValue.range.endLine - ptcValue.range.startLine + 1;
      const loadedWord = visibleLines === 1 ? "line" : "lines";
      const summaryParts: string[] = [
        info.truncated
          ? `loaded ${visibleLines} of ${ptcValue.truncation?.totalLines ?? ptcValue.range.totalLines} ${loadedWord} (truncated)`
          : `loaded ${visibleLines} ${loadedWord}`,
      ];
