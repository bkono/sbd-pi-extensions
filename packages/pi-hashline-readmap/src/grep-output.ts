...
function renderGroupHeader(group: GrepOutputGroup): string {
  ...
  const suffix =
    group.scope.contextLines !== undefined ? `, scoped to ±${group.scope.contextLines} lines` : "";
  ...
  return {
    ...
    groups: groups
      ...
      .map((group) => ({
        ...
        lineAnchors: group.entries.flatMap((entry) =>
          entry.kind === "separator" ? [] : [entry.line.anchor],
        ),
      })),
    warnings: [...warnings],
  };
...
export function buildGrepOutput(input: BuildGrepOutputInput): GrepOutputResult {
  ...
  for (const group of input.groups) {
    ...
    contextHygieneResources.push(
      buildSymbolResource(group.absolutePath, group.scope.symbol.name, group.scope.symbol.kind),
    );
  }
