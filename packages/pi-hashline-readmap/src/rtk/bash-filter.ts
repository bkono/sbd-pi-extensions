...
export function isGitCommand(command: string): boolean {
  return git.isGitCommand(command);
}
...
export function filterBashOutput(command: string, output: string): FilterResult {
  ...
  try {
    const routes: Array<{
      ...
      {
        matches: isGitCommand(command),
        technique: "git",
        apply: () => git.compactGitOutput(stripped, command),
      },
      {
        matches: isLinterCommand(command),
        technique: "linter",
        apply: () => linter.aggregateLinterOutput(stripped, command),
      },
      {
      ...
      {
        matches: isBuildCommand(command),
        technique: "build",
        apply: () => build.filterBuildOutput(stripped, command),
      },
      {
      ...
      {
        matches: docker.isDockerCommand(command),
        technique: "docker",
        apply: () => docker.compressDockerOutput(stripped),
      },
      {
      ...
      {
        matches: httpClient.isHttpCommand(command),
        technique: "http-client",
        apply: () => httpClient.compressHttpOutput(stripped),
      },
      {
        matches: transfer.isTransferCommand(command),
        technique: "transfer",
        apply: () => transfer.compressTransferOutput(stripped),
      },
    ];
