...
function renderPreview(options: {
  ...
  if (options.originalMetadata?.originalPath)
    rendered.push(`Original/pre-RTK output: ${options.originalMetadata.originalPath}`);
  if (options.originalMetadata) {
    rendered.push(
      `Original/pre-RTK: ${options.originalMetadata.originalLineCount} lines, ${options.originalMetadata.originalByteCount} bytes`,
    );
  }
  rendered.push(
  ...
  if (omitted.length > 0)
    rendered.push(`... omitted ${omitted.length} lines / ${byteCount(omittedText)} bytes ...`);
...
export function resolveBashContextGuardConfig(env: Env = process.env): BashContextGuardConfig {
  ...
  const enabled =
    env.PI_HASHLINE_BASH_CONTEXT_GUARD === "0"
      ? false
  ...
  return {
    ...
    maxLines: resolveDimension(
      env.PI_HASHLINE_BASH_CONTEXT_GUARD_MAX_LINES,
      settings?.maxLines,
      BASH_CONTEXT_GUARD_DEFAULT_MAX_LINES,
    ),
    maxBytes: resolveDimension(
      env.PI_HASHLINE_BASH_CONTEXT_GUARD_MAX_BYTES,
      settings?.maxBytes,
      BASH_CONTEXT_GUARD_DEFAULT_MAX_BYTES,
    ),
    headLines: resolveDimension(
      env.PI_HASHLINE_BASH_CONTEXT_GUARD_HEAD_LINES,
      settings?.headLines,
      BASH_CONTEXT_GUARD_DEFAULT_HEAD_LINES,
    ),
    tailLines: resolveDimension(
      env.PI_HASHLINE_BASH_CONTEXT_GUARD_TAIL_LINES,
      settings?.tailLines,
...
export function applyBashContextGuard(
  options: ApplyBashContextGuardOptions,
