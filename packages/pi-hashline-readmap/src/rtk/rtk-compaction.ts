...
function extractRouteTruncationMarkers(text: string): string[] {
  return ROUTE_TRUNCATION_MARKERS.flatMap((marker) =>
    Array.from(text.matchAll(marker), ([match]) => match.trim()),
  );
}
...
export function buildRtkCompaction(input: BuildRtkCompactionInput): RtkCompaction {
  ...
  const truncated =
    ROUTE_TRUNCATION_TECHNIQUES.has(info.technique) && modifiedOutput && introducedTruncationMarker;
