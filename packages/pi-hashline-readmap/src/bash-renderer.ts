...
const BASH_PARAMETERS = Type.Object({
  command: Type.String({
    description: "Test/build/git/pkg/external command; not repo file read/search/list/edit.",
  }),
...
export function registerBashRendererTool(
  ...
  options: { cwd?: string; createBuiltInBashTool?: BuiltInFactory } = {},
): any {
  const cache = new Map<string, any>();
  const createBuiltInBashTool =
    options.createBuiltInBashTool ?? ((cwd: string) => createBashTool(cwd));
  ...
  const tool = {
    ...
    renderCall(args: any, theme: any, context: any = {}) {
      ...
      return new Text(
        clampLineToWidth(
          `${renderToolLabel(theme, "bash")} ${theme.fg("muted", command)}`,
          context.width,
        ),
        0,
        0,
    ...
    renderResult(result: any, optionsArg: any, _theme: any, context: any = {}) {
      ...
      let rendered = summaryLine(`${lineCount} ${lineCount === 1 ? "line" : "lines"} returned`, {
        hidden: !expanded,
      });
