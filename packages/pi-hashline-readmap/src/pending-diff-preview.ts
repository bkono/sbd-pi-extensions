...
): { type: "ok"; path: string; existed: boolean } | { type: "skip"; reason: string } {
...
function readUtf8File(
  filePath: string,
): { type: "ok"; content: string } | { type: "skip"; reason: string } {
...
function isReplaceEdit(edit: unknown): edit is ReplaceEdit {
  return (
    ...
    typeof edit === "object" &&
    "replace" in edit &&
    typeof (edit as any).replace?.old_text === "string" &&
    typeof (edit as any).replace?.new_text === "string"
  );
}
...
function isAnchorEdit(edit: unknown): edit is AnchorEdit {
  return (
    !!edit &&
    typeof edit === "object" &&
    ("set_line" in edit || "replace_lines" in edit || "insert_after" in edit)
  );
}

function applyAnchoredPreview(
  content: string,
  edits: AnchorEdit[],
): { type: "ok"; content: string } | { type: "skip"; reason: string } {
  try {
    return {
      type: "ok",
      content: applyHashlineEdits(content, edits as HashlineEditItem[]).content,
    };
  } catch (err: any) {
...
function isReplaceSymbolEdit(edit: unknown): edit is ReplaceSymbolEdit {
  return (
    !!edit &&
    typeof edit === "object" &&
    "replace_symbol" in edit &&
    typeof (edit as any).replace_symbol?.symbol === "string" &&
    typeof (edit as any).replace_symbol?.new_body === "string"
  );
}

async function applyReplaceSymbolPreview(
  filePath: string,
  content: string,
  edit: ReplaceSymbolEdit,
): Promise<{ type: "ok"; content: string } | { type: "skip"; reason: string }> {
  try {
    if (!edit.replace_symbol.new_body.trim())
      return { type: "skip", reason: "replace_symbol new_body is empty" };
    ...
    if (probe.type !== "ok")
      return { type: "skip", reason: `symbol projection failed: ${probe.message}` };
...
export function buildPendingWritePreviewData(
  ...
  const previous = resolved.existed
    ? readUtf8File(resolved.path)
    : { type: "ok" as const, content: "" };
  ...
  return buildData(
    ...
    input.content,
    resolved.existed,
    resolved.existed ? "pending overwrite" : "pending create",
  );
}
...
export function buildWritePreviewKey(input: {
  path?: unknown;
  content?: unknown;
}): string | undefined {
...
export function resolvePendingDiffPreview<T extends PendingDiffPreviewResult>(
  context:
    | { state?: Record<string, PendingDiffPreviewCacheSlot<T>>; invalidate?: () => void }
    | undefined,
  stateKey: string,
  ...
  root[stateKey] ??= {} as PendingDiffPreviewCacheSlot<T>;
  ...
  try {
    ...
    const skipped = {
      type: "skip",
      reason: `projection failed: ${err?.message ?? String(err)}`,
    } as T;
    slot.data = skipped;
    return skipped;
  }
