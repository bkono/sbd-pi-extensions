...
export type RenderTuiDiffInput = {
  diffData: DiffData;
  width: number;
  theme: RendererTheme;
  expanded: boolean;
...
function hasOldSide(data: DiffData): boolean {
  return data.entries.some((e) => e.kind === "remove" || e.kind === "context");
}
...
function hunkCount(data: DiffData): number {
  return Math.max(
    1,
    data.blockRanges?.length ??
      (data.entries.some((e) => e.kind === "add" || e.kind === "remove") ? 1 : 0),
  );
}
function compactHeader(data: DiffData): string {
...
function textOf(entry: DiffEntry): string {
  return "text" in entry ? entry.text : "";
}
function padRightVisual(line: string, width: number): string {
...
function spans(theme: RendererTheme, spans: DiffSpan[] | undefined, fallback: string): string {
  return (
    spans
      ?.map((s) =>
        s.kind === "add"
          ? theme.fg("success", s.text)
          : s.kind === "remove"
...
function inlineText(input: RenderTuiDiffInput, index: number, entry: DiffEntry): string {
  const pair = input.diffData.inlineDiffs?.find((d) =>
    entry.kind === "remove"
      ? d.removeLineIndex === index
      : entry.kind === "add"
        ? d.addLineIndex === index
        : false,
  );
  if (!pair) return textOf(entry);
  return entry.kind === "remove"
    ? spans(input.theme, pair.removeSpans, textOf(entry))
    : entry.kind === "add"
      ? spans(input.theme, pair.addSpans, textOf(entry))
      : textOf(entry);
}
function unifiedRows(input: RenderTuiDiffInput, width: number): string[] {
  ...
  for (const [i, e] of input.diffData.entries.entries()) {
    ...
    const tinted = wrapWithHangingIndent(prefix, inlineText(input, i, e), width, {
      tint: (text) => tint(input.theme, e, text),
    });
    rows.push(...tinted);
  }
  return rows;
...
function splitRows(input: RenderTuiDiffInput, width: number): string[] {
  ...
  for (const [i, e] of input.diffData.entries.entries()) {
    if (e.kind === "remove") {
      const left = wrapWithHangingIndent(`▌- ${e.oldLine} │ `, inlineText(input, i, e), pane, {
        tint: (text) => tint(input.theme, e, text),
      });
      for (const line of left) rows.push(`${padRightVisual(line, pane)} │ ${blankPane}`);
    } else if (e.kind === "add") {
      const right = wrapWithHangingIndent(`▌+ ${e.newLine} │ `, inlineText(input, i, e), pane, {
        tint: (text) => tint(input.theme, e, text),
      });
      for (const line of right) rows.push(`${blankPane} │ ${line}`);
    } else if (e.kind === "context") {
      const left = wrapWithHangingIndent(`▌  ${e.oldLine} │ `, e.text, pane, {
        tint: (text) => tint(input.theme, e, text),
      });
      const right = wrapWithHangingIndent(`▌  ${e.newLine} │ `, e.text, pane, {
        tint: (text) => tint(input.theme, e, text),
      });
...
function hiddenHint(hiddenLines: number, hiddenHunks: number, width: number): string {
  const forms = [
    `… (${hiddenLines} more diff lines • ${hiddenHunks} more hunk${hiddenHunks === 1 ? "" : "s"} • Ctrl+O to expand)`,
...
export function renderTuiDiff(input: RenderTuiDiffInput): RenderTuiDiffOutput {
  ...
  if (!input.expanded)
    return {
      mode,
      width,
      lines: clampLinesToWidth(
        [...lines, hiddenHint(input.diffData.entries.length, hunkCount(input.diffData), width)],
        width,
      ),
    };
  ...
  const rows =
    mode === "split"
      ? splitRows(input, width)
      : mode === "compact"
        ? compactRows(input, width)
        : unifiedRows(input, width);
