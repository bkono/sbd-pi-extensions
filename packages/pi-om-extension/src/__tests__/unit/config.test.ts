...
describe("loadConfig", () => {
  ...
  it("returns defaults when no config files or env vars exist", () => {
    ...
    expect(config.observation.stageMessageTokens).toBe(96_000);
    expect(config.observation.publishMessageTokens).toBe(192_000);
    expect(config.observation.stageMessageCount).toBe(48);
    expect(config.observation.publishMessageCount).toBe(96);
    expect(config.observation.stageToolResultTokens).toBe(32_000);
    expect(config.observation.publishToolResultTokens).toBe(96_000);
    expect(config.observation.maxChunkMessageTokens).toBe(32_000);
    expect(config.observation.maxChunkMessages).toBe(32);
    expect(config.reflection.observationTokens).toBe(120_000);
